...
class AnimValue {
  ...
  private:
	double duration() const {
		TimePoint newtime = now();
		TimeSeconds t = newtime - m_time;
		m_time = newtime;
		return clamp(t.count(), 0.0, 1.0);
	}
	mutable double m_value;
	double m_target;
	double m_rate;
	mutable TimePoint m_time;
};
...
class AnimAcceleration {
  public:
	...
	double getValue() {
		...
		TimePoint curtime = now();
		double duration = (curtime - m_time).count();
		m_time = curtime;
